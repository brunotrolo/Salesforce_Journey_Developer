...
Before asking a human to Fetch in the Designer, prove the data layer directly (rodar contra `--target-org "$FSC_TARGET_ORG"`, com um recordId de teste da capacidade — nunca o Id de exemplo em produção):
...
- **Never write `style="..."` inside a `mergeField`.** `lightning-formatted-rich-text` (the runtime for a `Text`/`outputField` element) strips every inline `style` attribute from the HTML it renders — confirmed by DOM inspection, see `.claude/agent-memory/fsc-omnistudio-developer/MEMORY.md` §4. The structure still renders (tags/line breaks survive), so the visible symptom is unstyled plain text that looks like a CSS problem, not a stripped-attribute problem — don't mistake one for the other and don't re-derive this the hard way. Use real, verified SLDS utility classes in `class="..."` instead — apply `.claude/skills/salesforce/design-systems-slds-apply/SKILL.md`'s verified hooks/classes here too, not just for LWC. Never invent a class name; a plausible-looking SLDS class that doesn't exist fails silently (unstyled), the same failure shape as the stripped-style bug. For a color/value SLDS genuinely doesn't cover, put a `<style>` block in the state's own `styleObject` (card configuration, not merge-field payload) instead of inline `style=` — confirm via DOM inspection that it actually applies before trusting it.
