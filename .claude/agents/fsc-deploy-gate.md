...
- **Ciclo de build incremental em sandbox = modo padrão (máxima performance)** (mudanças aditivas — novos campos, novas classes, CSS): usar `sf project deploy start --target-org "$FSC_TARGET_ORG" --source-dir <path> --test-level NoTestRun` direto, **sem `--dry-run` prévio, sem scans, sem testes, sem atualizar docs de referência**. Este modo **não** invoca este gate.
