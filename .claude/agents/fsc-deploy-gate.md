...
- **Ciclo de build incremental em sandbox = modo padrão (máxima performance)** (mudanças aditivas — novos campos, novas classes, CSS): usar `sf project deploy start --target-org "$FSC_TARGET_ORG" --metadata <Tipo>:<Nome> --test-level NoTestRun` direto — **sempre `--metadata`, nunca `--source-dir`, neste modo** (múltiplos artefatos na mesma chamada: `--metadata Tipo1:Nome1,Tipo2:Nome2`; `guard-deploy.mjs` bloqueia `--source-dir` combinado com `--test-level NoTestRun`). Ex.: `sf project deploy start --target-org "$FSC_TARGET_ORG" --metadata ApexClass:LogEntryEventBuilder --test-level NoTestRun`. Sem `--dry-run` prévio, sem scans, sem testes, sem atualizar docs de referência. Este modo **não** invoca este gate.
...
6. **Permission set / access check, if the capability's users need new access**
   - Confirm the permission set generated by `fsc-data-model-developer` (the sole PS/PSG/CP author in this project) was actually deployed (part of step 3's payload) and, where the plan calls for it, assigned:
      ...
      ```
   - Assert internal naming conformance on every PS/PSG/CP this capability added (`PS_`/`PSG_`/`CP_` prefix, operation in English, access level strictly `Edit`|`View`|`Admin` per `.claude/skills/fsc-build/references/permissionamento-v2.md`) — a misnamed permission set routes back to `fsc-data-model-developer`, the same as any other defect.
