...
3. **Dispatch specialists in dependency order** (this mirrors `platform-metadata-deploy`'s own default deployment order — objects/fields before permission sets before Apex before Flow — so build order and deploy order don't fight each other):
   ...
   3. `fsc-lwc-developer` — production LWC components (never the Designer's prototype kit — see that agent's own scope note).
   4. `fsc-omnistudio-developer` — real FlexCard/OmniScript metadata (only FlexCard + OmniScript, same project-wide scope the Designer already fixed).
   5. `fsc-automation-developer` — Flow, if the plan calls for declarative automation.
