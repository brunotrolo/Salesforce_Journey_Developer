...
5. Generate the permission set(s) this capability's users need: object permissions, field-level security for every field you just added, and tab visibility if the plan calls for a new tab. **Naming follows the ODIN pattern — read `.claude/skills/fsc-build/references/permissionamento-odin-v2.md` before naming anything**: `[TipoArtefato]_[TipoProduto]_[Operação]_[Especificidade]_[NivelAcesso]` (e.g. `PS_PFCard_InvoiceCancellation_Edit`, `PS_Cross_CPFQuery_View`), Label and API Name in English, access level strictly `Edit` | `View` | `Admin`, one permission per operation — never a generic catch-all a permission set nobody can reason about from its name becomes unmaintainable exactly the way the source org already is.
